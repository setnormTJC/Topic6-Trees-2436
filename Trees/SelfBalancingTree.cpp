...
//see: https://github.com/Dynmi/RedBlackTree/blob/master/rbtree.h
#ifndef TREE_H_
#define TREE_H_
...
private:
	...
	int     height(rbNode<K, T>* node) const;
...
template<class K, class T>
int rbTree<K, T>::getTreeHeight() const {
	// Calls the recursive helper function starting from the root.
	return height(root);
}

template<class K, class T>
int rbTree<K, T>::height(rbNode<K, T>* node) const {
	if (node == nullptr) {
	...
	// The height of the current node is 1 + the maximum of the two subtree heights
